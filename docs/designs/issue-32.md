...
**テストカバレッジ率 100% を目標とする。** 全新規モジュール・改修箇所に対してユニットテストを網羅的に作成し、
ブランチカバレッジも含めて未テストのコードパスを残さない。

- `pytest-cov` を使用してカバレッジを計測する
- CI で `--cov-fail-under=100` を設定し、カバレッジ低下時にビルドを失敗させる
- 対象モジュール:
  - `notifications/templates.py` — 100%
  - `notifications/progress.py` — 100%
  - `notifications/error_summarizer.py` — 100%
  - `notifications/slack.py`（改修部分） — 100%
  - `models.py`（`NotificationType` 追加部分） — 100%

### 8.1 新規テストケース

#### 8.1.1 テンプレートレジストリ (`test_notification_templates.py`)

| テスト | 検証内容 |
|-------|---------|
| `test_notification_type_emoji_mapping` | 各 `NotificationType` に対応する絵文字が正しいこと |
| `test_all_notification_types_have_template` | 全 `NotificationType` 値に対応するテンプレートが存在すること |
| `test_template_color_format` | 全テンプレートの `color` が有効な hex カラーコードであること |
| `test_template_default_message_not_empty` | 全テンプレートの `default_message` が空でないこと |
| `test_template_immutability` | `NotificationTemplate` が frozen dataclass であり変更不可であること |

#### 8.1.2 Block Kit ペイロード構築 (`test_slack_payload.py`)
...
| `test_duration_format_edge_cases` | 0秒、59秒、60秒、3600秒等の境界値が正しくフォーマットされること |
| `test_done_summary_payload` | 完了サマリーにフェーズ別統計が含まれること |
| `test_done_summary_with_multiple_prs` | 設計PR・実装PR両方のボタンが表示されること |
| `test_files_changed_in_metadata` | 変更ファイル一覧がペイロードに含まれること |
| `test_next_action_in_context` | 次のアクション指示がコンテキストに含まれること |
| `test_repo_name_in_all_notifications` | 全通知タイプで `repo` 情報が含まれること |
| `test_issue_title_in_all_notifications` | 全通知タイプで Issue タイトルが含まれること |
| `test_block_count_within_limit` | ペイロードのブロック数が Slack の 50 ブロック制限内であること |
| `test_backward_compatibility` | `notification_type=None` で従来動作すること |
| `test_backward_compatibility_with_level` | 従来の `level` パラメータのみ指定時に従来ペイロードが生成されること |

#### 8.1.3 エラー要約 (`test_error_summarizer.py`)

| テスト | 検証内容 |
|-------|---------|
| `test_error_summary_format` | エラー要約が正しくフォーマットされること |
| `test_timeout_error_summary` | `TimeoutError` 用テンプレートが適用されること |
| `test_runtime_error_summary` | `RuntimeError` 用テンプレートが適用されること |
| `test_auth_error_summary` | `AuthError` 用テンプレートが適用されること |
| `test_git_conflict_error_summary` | `GitConflictError` 用テンプレートが適用されること |
| `test_unknown_error_fallback` | 未定義エラータイプでデフォルトテンプレートが適用されること |
| `test_error_summary_with_phase_info` | フェーズ情報付きのエラー要約が正しいこと |
| `test_error_summary_with_issue_number` | Issue 番号付きのエラー要約が正しいこと |
| `test_traceback_tail_extraction` | トレースバック末尾3行が正しく抽出されること |
| `test_format_for_slack_mrkdwn` | Slack mrkdwn 形式への変換が正しいこと |

#### 8.1.4 進捗トラッカー (`test_progress_tracker.py`)

| テスト | 検証内容 |
|-------|---------|
| `test_progress_tracker_lifecycle` | ProgressTracker の start → update → complete → summary |
| `test_start_phase` | フェーズ開始が正しく記録されること |
| `test_update_step` | ステップ更新が正しく反映されること |
| `test_update_step_invalid_phase` | 未開始フェーズのステップ更新でエラーにならないこと |
| `test_complete_phase` | フェーズ完了が正しく記録され、結果が返ること |
| `test_complete_phase_with_cost` | コスト情報付きのフェーズ完了が正しいこと |
| `test_get_progress_existing` | 存在するフェーズの進捗が取得できること |
| `test_get_progress_nonexistent` | 存在しないフェーズで `None` が返ること |
| `test_get_summary_single_phase` | 単一フェーズのサマリーが正しいこと |
| `test_get_summary_multiple_phases` | 複数フェーズのサマリー（合計時間・合計コスト）が正しいこと |
| `test_cleanup` | cleanup 後にデータが削除されること |
| `test_cleanup_nonexistent_issue` | 存在しない Issue の cleanup でエラーにならないこと |
| `test_multiple_issues_isolation` | 複数 Issue の追跡データが互いに影響しないこと |

#### 8.1.5 フェーズ通知統合テスト (`test_phase_notifications.py`)

| テスト | 検証内容 |
|-------|---------|
| `test_phase_start_notification` | フェーズ開始通知が送信されること |
| `test_phase_start_notification_all_phases` | 全フェーズで開始通知が送信されること |
| `test_hearing_notification_type` | hearing 完了時に `HEARING_QUESTION` タイプで通知されること |
| `test_design_notification_with_pr_url` | design 完了時に PR URL 付きで通知されること |
| `test_implement_notification_with_diff_stats` | implement 完了時に変更ファイル情報付きで通知されること |
| `test_done_notification_with_summary` | done フェーズで完了サマリーが送信されること |
| `test_error_notification_with_summary` | エラー時に ErrorSummarizer 経由で通知されること |
| `test_timeout_notification` | タイムアウト時に `TIMEOUT` タイプで通知されること |

#### 8.1.6 ユーティリティ (`test_notification_utils.py`)

| テスト | 検証内容 |
|-------|---------|
| `test_get_diff_stats_success` | `git diff --stat` の結果が正しくパースされること |
| `test_get_diff_stats_failure` | git コマンド失敗時に空結果が返ること（best-effort） |
| `test_get_diff_stats_empty_diff` | 差分がない場合に空結果が返ること |
| `test_phase_labels_complete` | 全フェーズ名に対応するラベルが定義されていること |
| `test_phase_steps_complete` | 全フェーズに対応するステップが定義されていること |
| `test_workflow_phases_complete` | 全ワークフロータイプのフェーズ順序が定義されていること |

### 8.2 既存テストの更新

- `TC-SL-01` ～ `TC-SL-10`: 既存テストは後方互換性により引き続きパスすることを確認
- 必要に応じてアサーションを追加
- 既存テストも新しいカバレッジ基準に合わせてアサーションを強化する
