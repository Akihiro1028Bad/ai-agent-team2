...
- CI実行開始 / CI失敗検知のポーリング追加（新規イベント検知ロジック） → **Issue #11**
- PRマージ完了のポーリング追加 → **Issue #11**
- レビューコメント検知のポーリング追加 → **Issue #11**
- 承認待ちリマインダー通知 → **Issue #11**

> 上記の新規イベント検知ロジックは Issue #11 に分離済み。本Issueでは通知フォーマット・内容の改善と、
...
本Issueのスコープ外だが、以下の拡張を別Issueで対応予定（Issue作成済み）:

1. **新規イベント検知 (ポーリング追加)** → **Issue #11**
