...
class TestBugWorkflow:
    ...
    async def test_bug_with_ci_fix(self, sm: StateMachineManager) -> None:
        ...
        await sm.transition(2, Phase.CI_FIX)  # self-transition
    ...
    async def test_bug_plan_rejected(self, sm: StateMachineManager) -> None:
        ...
        await sm.transition(3, Phase.ANALYSIS)  # rejected
    ...
    async def test_bug_with_impl_revise(self, sm: StateMachineManager) -> None:
        ...
        sm.set_issue_type(4, "bug")

        await sm.transition(4, Phase.ANALYSIS)
        await sm.transition(4, Phase.PLAN_REVIEW)
        await sm.transition(4, Phase.FIX)
        await sm.transition(4, Phase.IMPL_REVIEW)
        await sm.transition(4, Phase.IMPL_REVISE)
        await sm.transition(4, Phase.IMPL_REVIEW)
...
class TestFeatureMWorkflow:
    ...
    async def test_feature_m_design_revise(self, sm: StateMachineManager) -> None:
        ...
        await sm.transition(21, Phase.DESIGN_REVISE)
        await sm.transition(21, Phase.DESIGN_REVIEW)
    ...
    async def test_feature_m_with_ci_fix(self, sm: StateMachineManager) -> None:
        ...
        sm.set_issue_type(22, "feature-m")

        await sm.transition(22, Phase.HEARING)
        await sm.transition(22, Phase.DESIGN)
        await sm.transition(22, Phase.DESIGN_REVIEW)
        await sm.transition(22, Phase.PLANNING)
        await sm.transition(22, Phase.IMPLEMENT)
        await sm.transition(22, Phase.CI_FIX)
        await sm.transition(22, Phase.IMPL_REVIEW)
        await sm.transition(22, Phase.DONE)
        assert sm.get_phase(22) == Phase.DONE
...
class TestFeatureLWorkflow:
    ...
    async def test_feature_l_split_modified(self, sm: StateMachineManager) -> None:
        ...
        await sm.transition(31, Phase.HEARING)  # modified
...
class TestInvalidTransitions:
    ...
    async def test_type_detection_to_hearing_without_type_raises(self, sm: StateMachineManager) -> None:
        ...
        with pytest.raises(InvalidTransitionError):
            await sm.transition(43, Phase.HEARING)
...
class TestPersistence:
    ...
    def test_load_from_persistence(self, mock_persistence: MagicMock, mock_tracker: AsyncMock) -> None:
        ...
        assert manager.get_phase(1) == Phase.DESIGN
    ...
    def test_load_multiple_issues(self, mock_persistence: MagicMock, mock_tracker: AsyncMock) -> None:
        ...
        manager.load_from_persistence()
        assert manager.get_phase(1) == Phase.ANALYSIS
        assert manager.get_phase(2) == Phase.HEARING
        assert manager.get_issue_type(1) == "bug"
        assert manager.get_issue_type(2) == "feature-m"
...
class TestSuspendedResume:
    ...
    async def test_resume_from_suspended_to_analysis(self, sm: StateMachineManager) -> None:
        ...
        await sm.transition(61, Phase.ANALYSIS)
        await sm.transition(61, Phase.SUSPENDED)
    ...
    async def test_resume_from_suspended_to_implement(self, sm: StateMachineManager) -> None:
        ...
        await sm.transition(62, Phase.HEARING)
        await sm.transition(62, Phase.SUSPENDED)

        await sm.transition(62, Phase.IMPLEMENT)
        assert sm.get_phase(62) == Phase.IMPLEMENT

    @pytest.mark.asyncio
    async def test_suspend_from_design(self, sm: StateMachineManager) -> None:
        """DESIGN -> SUSPENDED."""
        sm.register_issue(63, "owner/repo")
        sm.set_issue_type(63, "feature-m")
        await sm.transition(63, Phase.HEARING)
        await sm.transition(63, Phase.DESIGN)
        await sm.transition(63, Phase.SUSPENDED)
        assert sm.get_phase(63) == Phase.SUSPENDED
...
class TestCiRetry:
    ...
    async def test_ci_retry_unregistered_issue(self, sm: StateMachineManager) -> None:
        ...
        assert await sm.get_ci_retry_count(999) == 0
...
class TestTrackerIntegration:
    ...
    async def test_tracker_called_on_transition(self, sm: StateMachineManager, mock_tracker: AsyncMock) -> None:
        ...
        await sm.transition(90, Phase.ANALYSIS)
